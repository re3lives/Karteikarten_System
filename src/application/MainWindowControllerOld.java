package application;

import javafx.application.Platform;
import javafx.collections.FXCollections;
import javafx.collections.ObservableList;
import javafx.fxml.FXML;
import javafx.fxml.FXMLLoader;
import javafx.scene.Parent;
import javafx.scene.Scene;
import javafx.scene.control.ListView;
import javafx.stage.Stage;
/*
public class MainWindowControllerOld {
	
	public void closeApp() {
		Platform.exit();
		System.exit(0);
	}
	
	public void openAbout() {
		
		try {
			Parent root = FXMLLoader.load(getClass().getResource("about.fxml"));

			Scene scene = new Scene(root, 600, 300);

			Stage stage = new Stage();
			stage.setTitle("Über uns");
			stage.setScene(scene);  
	        stage.show();
	        //testSubjectViewManager();
		} catch (Exception e) {
			e.printStackTrace();
		}
		
	}
	
	/*private class SubjectViewManager {
		@FXML private ListView<String> subjectListView;
		
		public SubjectViewManager() {
			ObservableList<String> list = FXCollections.observableArrayList("moin", "gutenTag");
			subjectListView.setItems(list);
		}
	}
	
	/*
	private class VocabViewManager{
		
	}
	
	private class VocabTestManager{
		
	}*/
//}
